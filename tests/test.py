...
class TestData(unittest.TestCase):
    def test_data(self) -> None:
        ...
        data_from_output_files = CivetData.from_output_files(
            Path(DUMMY_DATA_PATHS["dir"])
        )
        self.assertTrue(
            np.array_equal(
                data_from_csv.subject_ids, data_from_output_files.subject_ids
            )
        )
        self.assertTrue(
            np.array_equal(data_from_csv.features, data_from_output_files.features)
        )
...
class TestMain(unittest.TestCase):
    ...
    def test_input_path_does_not_exist(self) -> None:
    ...
    @patch(
        ...
        [
            "civetqc",
            DUMMY_DATA_PATHS["csv"],
            "--output_dir",
            "probably_not_a_directory",
        ],
    )
    def test_output_dir_does_not_exist(self) -> None:
        self.assertRaises(NotADirectoryError, main)

    @patch(
        "sys.argv",
        ["civetqc", DUMMY_DATA_PATHS["csv"], "--output_dir", TEST_OUTPUT_DIR],
    )
    def test_valid_args(self) -> None:
        ...
        data = QCRatingsData.from_csv(
            output_file, qcvar="RATING", allow_non_numeric=True
        )
...
class TestUtils(unittest.TestCase):
    def test_get_non_unique(self) -> None:
    ...
    def test_check_type(self) -> None:
        self.assertRaises(
            TypeError, utils.check_types, (5, int), ([1, 2, 3], np.ndarray)
        )
